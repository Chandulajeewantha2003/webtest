...
define( 'LD_ASSETS_CSS', plugins_url( '/assets/css', __FILE__ ) );
define( 'LD_ASSETS_VENDORS', plugins_url( '/assets/vendors', __FILE__ ) );
define( 'LD_ASSETS_JS', plugins_url( '/assets/js', __FILE__ ) );
...
class Liquid_Addons extends Liquid_Base {
	...
	public function __construct() {
		...
		$this->includes();
		$this->add_action( 'init', 'load_plugin_textdomain' );
	...
	public function enqueue() {
		...
		//Load jquery UI css
		wp_enqueue_style('jquery-ui', LD_ASSETS_CSS. '/jquery-ui.min.css' );
		wp_enqueue_style('jquery-ui-structure', LD_ASSETS_CSS. '/jquery-ui.structure.min.css' );
		wp_enqueue_style('jquery-ui-theme', LD_ASSETS_CSS. '/jquery-ui.theme.min.css' );

		//Animated icon font
		wp_enqueue_style( 'liquid-animated-icons', LD_ASSETS_VENDORS . '/animated-icons/style.css' );

		wp_enqueue_style( 'ld-colorpicker',    LD_ASSETS_CSS. '/liquid-colorpicker.css' );
		wp_enqueue_script( 'liquid-grapick',   LD_ASSETS_VENDORS . '/grapick/grapick.min.js' ,  array('jquery'), '1.0.0', true );
		wp_enqueue_script( 'liquid-colorpicker',   LD_ASSETS_JS . '/plugin.liquidColorPicker.min.js' ,  array('jquery'), '1.0.0', true );
		wp_enqueue_script( 'ld-vc-script',   LD_ASSETS_JS . '/vc-script.js' , array('jquery'), '1.0.0', true );

		if( function_exists( 'vc_mode' ) && 'admin_frontend_editor' === vc_mode() ) {
			wp_enqueue_script( 'ld-vc-frontend-script',   LD_ASSETS_JS . '/vc-script-frontend.js' , array( 'jquery', 'ld-vc-script' ), '1.0.0', true );
			wp_enqueue_style( 'lqd-admin-frontend', LD_ASSETS_CSS. '/liquid-vc.min.css' );
		}
	...
	public function vc_frontend_jscss() {
		wp_enqueue_style( 'ld-vc-frontend', LD_ASSETS_CSS. '/vc-frontend-style.min.css' );
	}

	public function vc_liquid_css() {
		wp_enqueue_style( 'lqd-admin', LD_ASSETS_CSS. '/liquid-vc.min.css', array( 'js_composer' ) );
		if( is_rtl() ) {
			wp_enqueue_style( 'lqd-admin-rtl', LD_ASSETS_CSS. '/liquid-vc-rtl.min.css', array( 'lqd-admin' ) );
		}
	}

	public function redux_liquid_css() {
		wp_enqueue_style( 'lqd-redux', LD_ASSETS_CSS. '/liquid-redux.min.css', array() );
		if( is_rtl() ) {
			wp_enqueue_style( 'lqd-redux-rtl', LD_ASSETS_CSS. '/liquid-redux-rtl.min.css', array() );
		}
	}

	public function plugin_css() {
		wp_enqueue_style( 'ave-core', LD_ASSETS_CSS. '/ave-core.min.css', array( 'js_composer_front' ) );
	...
	public function enqueue_widgets() {
		//wp_enqueue_script( 'rs-widgets',   LD_ASSETS_JS . '/widgets.js' ,  array('jquery','select2'), '1.0.0', true );
		wp_enqueue_media();
	}
